GomokuGame.ai = (function() {
                ...
                for (steps = 1; steps <= 2; steps += 1) {
                    ...
                    for (side = -1; side <= 1; side += 1) {
                        ...
                        if (!(steps === 2 && side === 0)) {
                            ...
                            nextRow = move.row + (Math.abs(side) *
                                (directions[direction][side + 2] * (steps * 2)));
                            nextColumn = move.column + (Math.abs(side) *
                                (directions[direction][side + 1] * (steps * 2)));

                            nNextRow = move.row + (Math.abs(side) *
                                (directions[direction][side + 2] * (steps * 3)));
                            nNextColumn = move.column + (Math.abs(side) *
                                (directions[direction][side + 1] * (steps * 3)));

                            if (board[row] !== undefined &&
                                board[row][column] !== undefined &&
                                nextRow >= 2 && nextRow <= 16 &&
                                nNextRow >= 2 && nNextRow <= 16 &&
                                nextColumn >= 2 && nextColumn <= 16 &&
                                nNextColumn >= 2 && nNextColumn <= 16) {
                                // Check that spot exists && it is not already filled
                                ...
                                if (spot[direction] !== null) {

                                    if (board[move.row + (Math.abs(side) *
                                                ((directions[direction][side + 2] + 1) * steps))]
                                        [move.column + (Math.abs(side) *
                                                ((directions[direction][side + 1] + 1) * steps))] === null) {
                                  ...
                                  //          spot[direction][0] === 36 || spot[direction][0] === 40 ||
                                  //          spot[direction][0] === 82 || spot[direction][0] === 84 ||
                                  //          spot[direction][0] === 90 || spot[direction][0] === 94 ||
                                  //          spot[direction][0] === 108 || spot[direction][0] === 112 ||
                                  //          spot[direction][0] === 118 || spot[direction][0] === 120) &&
                                  //          move.colour === 1) ||
    ...
    var getScore = function(row, column) {
        ...
        if (board[row][column][5] !== null) {
            for (i = 0; i <= 3; i += 1) {
                for (j = 0; j <= 4; j += 1) {
                    if (board[row][column][4][i][j][0] !== null) {
                        board[row][column][4][4] += Math.abs(board[row][column][4][i][j][0]);
                    }
                }
            }
        } else {
    ...
    ai.initialise = function(columns, rows, depth) {
        ...
        for (i = 0; i <= rows + 4; i += 1) {
            ...
            for (j = 0; j <= columns + 4; j += 1) {
                ...
                if (i <= 2 && j <= 2 || i >= rows + 2 && j >= columns + 2) {
                  temp[2][0] = null;
                  temp[3][1][0] = null;
                }

                if (i <= 2 && j >= columns + 2 || i >= rows + 2 && j <= 2) {
                  temp[0][0] = null;
                  temp[3][1][0] = null;
                }
    ...
    ai.restart = function() {
        var height = board.length - 3;
        var width = board[0].length - 3;
        board = [];
        ai.initialise(height, width, depth);
    };
